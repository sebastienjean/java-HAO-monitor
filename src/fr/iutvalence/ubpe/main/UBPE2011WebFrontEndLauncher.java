...
public class UBPE2011WebFrontEndLauncher
{
	...
	{	
		...
		new Thread(new JsonFileTokensRemoverPeriodicService(30000, new File(args[2]+".json"), "UTF-8", new File(args[2]+".clean"), new String[] {"<!-- @@EVENT@@ -->"})).start();
		System.out.println("... done");		
		
		System.out.println("Starting JSON filtered producer periodic service ...");
		new Thread(new JsonFilteredFileProducerService(30000, new File(args[2]+".clean"), "UTF-8", new File("global_analogTempVersusTime.json"), new int[] {1, 3, 15})).start();
		System.out.println("... done");		
